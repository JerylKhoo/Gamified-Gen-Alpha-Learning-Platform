...
// Formats a course_id like "rizz-theory" or "MEME_LINGUISTICS_101" → "Rizz Theory" / "Meme Linguistics 101"
...
function CourseCard({ course, onClick, completed }) {
  const { emoji, bg, pattern } = COURSE_VISUAL;
  ...
  return (
    ...
    >
      ...
      <div className={`relative bg-gradient-to-br ${bg} flex-shrink-0 overflow-hidden h-[260px]`}>
        {course.image ? (
          <>
            <img src={course.image} alt={title} className="absolute inset-0 w-full h-full object-cover" />
    ...
    >
      ...
      >
        ...
        <div className={`relative h-[180px] bg-gradient-to-br ${bg} overflow-hidden`}>
          {course.image ? (
            <img src={course.image} alt={title} className="absolute inset-0 w-full h-full object-contain" />
          ) : (
            <>
              <div className="absolute inset-0" style={{ background: pattern }} />
              <div className="absolute inset-0 opacity-[0.06]"
        ...
        <div className="p-6 flex flex-col gap-4">
          ...
          <button
            onClick={() => navigate(`/home/learn/${course.courseId}`)}
...
export default function LearnPage() {
  ...
  const [completedCourses, setCompleted]    = useState(new Set());
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // Fetch courses and progress records in parallel
        ...
        // Build a Set of completed courseIds from progress records
        if (progressRes.ok) {
          const progressData = await progressRes.json();
          const done = new Set(progressData.map(p => p.courseId));
          setCompleted(done);
        }
      } catch (err) {
          ...
          <div className="grid grid-cols-3 gap-5 xl:grid-cols-3 lg:grid-cols-2 md:grid-cols-2 sm:grid-cols-1">
            {courses.map(course => (
              <CourseCard
                key={course.courseId}
                course={course}
                completed={completedCourses.has(course.courseId)}
